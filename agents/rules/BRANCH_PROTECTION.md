...
## Sincronización con Upstream (cal.com)

Para sincronizar cambios de Cal.com sin traer todas las branches:

```bash
# Solo traer la branch main de upstream
git fetch upstream main:refs/remotes/upstream/main

# Luego hacer merge desde tu branch main local
git checkout main
git merge upstream/main --no-edit
```

**Importante:** NO ejecutar `git fetch upstream` sin especificar la branch,
ya que traerá las 800+ branches del repositorio original.

Si accidentalmente trajiste todas las branches, puedes limpiar con:
```bash
